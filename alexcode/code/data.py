...
def map_studies_results():
    ...
    """
    id_to_results = dict()
    train_csv = open('../input/train.csv')
...
def write_train_npy():
    ...
    study_ids, images = load_images('../input/train')  # load images and their ids
    ...
    y = np.array(y)
    np.save('../input/X_train.npy', X)
    np.save('../input/y_train.npy', y)
    print('Done.')
...
def write_validation_npy():
    ...
    ids, images = load_images('../input/validate')
    study_ids = []
    X = []
    ...
    X = np.array(X, dtype=np.uint8)
    np.save('../input/X_validate.npy', X)
    np.save('../input/ids_validate.npy', study_ids)
    print('Done.')
